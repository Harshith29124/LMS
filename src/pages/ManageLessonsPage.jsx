...
import { ArrowLeft, Plus, Trash2, HelpCircle, CheckCircle, Video, FileText, ChevronRight, Clock, Hash, Layout, Type } from 'lucide-react'
...
export default function ManageLessonsPage() {
  ...
  const handleAddLesson = async (e) => {
    ...
    try {
      ...
      toast.success('Asset integrated into curriculum')
    } catch (err) {
  ...
  const handleAddQuiz = async (e) => {
    ...
    if (quizForm.options.some((o) => !o.trim())) return toast.error('All four options are mandatory')
    if (!quizForm.correctAnswer) return toast.error('Designate the correct answer')
    setSubmitting(true)
    try {
      await quizAPI.create(selectedLesson._id, quizForm)
      setQuizModal(false)
      setQuizForm({ question: '', options: ['', '', '', ''], correctAnswer: '' })
      toast.success('Knowledge check created')
    } catch (err) {
    ...
    <div className="max-w-4xl mx-auto space-y-10 pb-20">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2">
        <div className="flex items-center gap-6 min-w-0">
            <button 
              className="p-3 bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 rounded-2xl border border-black/5 dark:border-white/5 text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all active:scale-95" 
              onClick={() => navigate('/instructor')}
            >
              <ArrowLeft size={20} />
            </button>
            <div className="min-w-0">
              <h1 className="text-3xl font-black text-slate-900 dark:text-white truncate">Curriculum Console</h1>
      ...
      <div className="space-y-4">
        {lessons.length > 0 ? (
          <AnimatePresence mode="popLayout">
            {lessons.map((lesson, index) => (
              <motion.div
                ...
                className="glass-panel group p-6 rounded-[2.5rem] flex items-center gap-6 border-black/5 dark:border-white/5 hover:bg-black/5 dark:hover:bg-white/5 transition-all"
              >
                <div className="w-14 h-14 bg-gradient-to-br from-brand-primary/10 to-brand-secondary/10 rounded-[1.25rem] border border-black/5 dark:border-white/5 flex items-center justify-center flex-shrink-0 group-hover:border-brand-primary/20 transition-all">
                  <span className="text-xl font-black text-brand-primary">{String(index + 1).padStart(2, '0')}</span>
                </div>

                <div className="flex-1 min-w-0">
                  <h3 className="text-xl font-bold text-slate-900 dark:text-white group-hover:text-brand-primary transition-colors truncate">{lesson.title}</h3>
                  <div className="flex flex-wrap items-center gap-6 mt-1">
                    {lesson.videoUrl && (
                      <span className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-500 group-hover:text-slate-700 dark:group-hover:text-slate-300">
                ...
                <div className="flex items-center gap-2">
                  <button
                    className="p-3 bg-black/5 dark:bg-white/5 hover:bg-brand-primary/20 border border-black/5 dark:border-white/5 hover:border-brand-primary/30 rounded-2xl text-slate-400 hover:text-brand-primary transition-all active:scale-95 group-hover:shadow-[0_0_15px_rgba(99,102,241,0.15)]"
          ...
          <div className="glass-panel border-dashed border-black/10 dark:border-white/10 rounded-[3rem] p-20 text-center space-y-8">
            <div className="w-24 h-24 bg-black/5 dark:bg-white/5 rounded-full flex items-center justify-center mx-auto text-slate-400 dark:text-slate-600">
               <Layout size={48} />
            </div>
            <div className="max-w-xs mx-auto space-y-2">
              <h3 className="text-2xl font-bold text-slate-900 dark:text-white">Curriculum Empty</h3>
      ...
      <Modal open={lessonModal} onClose={() => setLessonModal(false)} title="Module Deployment">
        <form onSubmit={handleAddLesson} className="space-y-6">
          ...
          <div className="flex gap-4 pt-4">
            <button type="button" className="flex-1 px-8 py-4 rounded-2xl bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-slate-600 dark:text-slate-400 font-bold transition-all active:scale-95" onClick={() => setLessonModal(false)}>
              Discard
            </button>
      ...
      <Modal open={quizModal} onClose={() => setQuizModal(false)} title={`Assessment: ${selectedLesson?.title}`} maxWidth={600}>
        <form onSubmit={handleAddQuiz} className="space-y-8">
          ...
          <div className="space-y-4">
                ...
                <button
                  ...
                  className={`w-12 h-12 rounded-xl flex items-center justify-center font-black transition-all active:scale-90 flex-shrink-0 border-2 ${
                    quizForm.correctAnswer === quizForm.options[i] && quizForm.options[i] 
                      ...
                      : 'bg-black/5 dark:bg-white/5 border-black/10 dark:border-white/5 text-slate-500 hover:border-brand-primary/30 group-hover:text-brand-primary'
                  }`}
                >
            ...
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest text-center pt-4 italic">
              * Click the letter circle to mark the correct answer
            </p>
          </div>

          <div className="flex gap-4">
            <button type="button" className="flex-1 px-8 py-4 rounded-2xl bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-slate-600 dark:text-slate-400 font-bold transition-all active:scale-95" onClick={() => setQuizModal(false)}>
              Discard
            </button>
            <button type="submit" className="flex-[2] premium-button" disabled={submitting}>
              {submitting ? 'Registering...' : 'Save Quiz'}
            </button>
          </div>
        </form>
      </Modal>
